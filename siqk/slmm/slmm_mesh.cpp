...
void make_cubedsphere (Vec3s::HostMirror& p, Idxs::HostMirror& e, const Int n) {
...
void make_cgll_from_geo (
  const Vec3s::HostMirror& geo_p, const Idxs::HostMirror& geo_c2n, const Int np,
  Vec3s::HostMirror& cgll_p, Idxs::HostMirror& cgll_c2n)
{
  Idxs::HostMirror geo_c2e, geo_e2n;
  ...
  ko::resize(cgll_p,
             ...
             3);
  ko::resize(cgll_c2n, nslices(geo_c2n), siqk::square(np));
  Int pi = 0;
  // Geo cell vertices.
  for ( ; pi < nslices(geo_p); ++pi)
    for (Int k = 0; k < 3; ++k) cgll_p(pi,k) = geo_p(pi,k);
  ko::View<int**, ko::HostSpace> nodes("nodes", np, np);
  const Real* gll_x = nullptr, * gll_wt = nullptr;
  GLL gll;
  ...
  for (Int gci = 0; gci < nslices(geo_c2n); ++gci) {
    ...
    for (Int ei = 0; ei < 4; ++ei) {
      ...
      // Add the new nodes.
      const auto p0 = slice(cgll_p, edge[0]);
      const auto p1 = slice(cgll_p, edge[np-1]);
      for (Int i = 1; i < np-1; ++i) {
        auto p = slice(cgll_p, pi);
        const Real alpha = 0.5*(gll_x[i] + 1);
        for (Int k = 0; k < 3; ++k)
  ...
  for (Int gci = 0; gci < nslices(geo_c2n); ++gci) {
    ...
    for (Int j = 1; j < np-1; ++j) {
      const auto p0 = slice(cgll_p, nodes(0,j));
      const auto p1 = slice(cgll_p, nodes(np-1,j));
      for (Int i = 1; i < np-1; ++i) {
        assert(pi < nslices(cgll_p));
        auto p = slice(cgll_p, pi);
        const Real alpha = 0.5*(gll_x[i] + 1);
        for (Int k = 0; k < 3; ++k)
          p[k] = (1 - alpha)*p0[k] + alpha*p1[k];
  ...
  // Project to the unit sphere.
  for (Int i = 0; i < nslices(cgll_p); ++i)
    geometry::normalize(slice(cgll_p, i));
}
  ...
  Idxs::HostMirror& cgll_io_c2n)
{
  ...
  ko::resize(cgll_io_c2n, nslices(cgll_c2n)*nsc, 4);
  ...
  IdxArray::HostMirror& dglln2cglln, Idxs::HostMirror& dgll_c2n)
{
  ...
  ko::resize(dglln2cglln, np2*nslices(cgll_c2n));
  ko::resize(dgll_c2n, nslices(cgll_c2n), szslice(cgll_c2n));
  IdxArray::HostMirror cgll_c2n_used("used", nslices(cgll_p));
  ko::deep_copy(cgll_c2n_used, 0);
...
void make_c2e_from_c2n (const Int np, const Idxs::HostMirror& c2n,
  ...
  ko::resize(c2e, nslices(c2n), szslice(c2n));
  ko::resize(e2n, nmbr, np);
  for (Int ci = 0; ci < nslices(c2n); ++ci) {
    const auto cell = slice(c2n, ci);
    for (Int ni = 0; ni < nnode; ++ni) {
      Edge e(cell[ni], cell[(ni+1) % nnode]);
